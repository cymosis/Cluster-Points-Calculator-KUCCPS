...
if len(selected_subjects) not in [7, 8]:
    ...
    cluster_groups_list = [cluster_1(grades, groups), cluster_2(grades_1, groups), cluster_3(grades_2, groups), cluster_4(grades_3, groups), cluster_9(grades_8, groups)]
   # Iterate through the cluster responses
    for i, total_points_all in enumerate(cluster_groups_list):
        ...
        st.write(f"{cluster_name}: points: {total_points_all_value:.2f}, cluster: {cluster:.2f}")

...
with col1:
    ...
    if len(selected_subjects) not in [7, 8]:
        ...
        grades_13 = grades.copy()
        grades_14 = grades.copy()
        grades_15 = grades.copy()
        grades_16 = grades.copy()
        grades_17 = grades.copy()
        grades_18 = grades.copy()
...
with col2:
    ...
    cluster_groups_list = [cluster_1(grades, groups), cluster_2(grades_1, groups), cluster_3(grades_2, groups), cluster_4(grades_3, groups),cluster_5(grades_4, groups),
                           ...
                           cluster_12(grades_11, groups),cluster_13(grades_12, groups),cluster_14(grades_13, groups),cluster_15(grades_14, groups),cluster_16(grades_15, groups),
                           cluster_17(grades_16, groups),cluster_18(grades_17, groups),cluster_19(grades_18, groups)]
   # Iterate through the cluster responses
    for i, total_points_all in enumerate(cluster_groups_list):
        ...
        missing_subjects = [subj for subj in total_points_all.get('subjects_needed', []) if subj not in grades]
        if missing_subjects:
            st.warning(f"The following subjects are missing for {cluster_name}: {', '.join(missing_subjects)}")
            continue 
